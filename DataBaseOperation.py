...
class DBOperation():
    ...
    def __init__(self):
        try:
            ...
            file.close()
        #self.mydb =mysql.connector.connect(host = "localhost",user = datadic['username'],password = datadic['password'],database = datadic['database'])
            self.mydb = mysql.connector.connect(host="localhost",user="vehicle_user",passwd="vehicle_password",database="vehicle_parking")
        except:
    ...
    def getSlotSpace(self):
        try:
            ...
            print("Error occured while fetching solts")

    def getCurrentVehicle(self):
        try:
            cursor = self.mydb.cursor()
        #is_exit = 0 vehicle in parking
            cursor.execute("select * from vehicles where is_exit = '0'")
            data = cursor.fetchall()
            cursor.close()
            return data
        except:
            print("Error occured while fetching Vehicles")

    def getAllVehicle(self):
        try:
            cursor = self.mydb.cursor()
        #is_exit = 1 vehicle slot avaialable
    ...
    def AddVehicles(self,name,vehicleno,mobile,vehicle_type):
        try:
            ...
            print("Error occured while parking the vehicle")
    ...
    def exitVehicle(self,id):
        try:
            ...
            print("Error occured while exiting")
